...
class ticket :
    _tickets = []
    def __init__(self,date_time ,user_id ,ticket_dapar ,ticket_arrival ,level = None) :
...
class Order(ticket) :
    _orders = []
    def __init__(self ,date_time ,user_id ,ticket_departure ,ticket_arrival ,level = None) :
        super().__init__(date_time,user_id,ticket_departure,ticket_arrival,level)
    ...
    def __call__(self) :
        ...
        lst = [self.date_time,self.user_id,self.departure,self.arrival,self.level]
        return lst
    ...
    when python wants to hash in a list it checks the equality of objects in the list and if they are 
...
def order_manager(date_time,ticket_user_id,ticket_departure,ticket_destination,order = None) :
    ...
    new_ticket = ticket(date_time,ticket_user_id,ticket_departure,ticket_destination,order)
...
for order in Order._orders :
    if order_manager(*order()) :       
        print("Movafagh")
    else :
        print("Na Movafagh")
